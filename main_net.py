...
from FSLiteNet import FSLiteNet, FSFBNet, DFIBNet
...
def get_args_parser():
    ...
    # Model parameters
    parser.add_argument('--arch', default='FSLiteNet', type=str,
                        help='gfnet-xs,combined-gfnet-residual,FSFBNet,FSLiteNet,FSLiteNetF2')
    ...
    parser.add_argument('--teacher-model', default='vgg16', type=str, metavar='MODEL',
                        help='Name of teacher model to train (default: "regnety_160"')
    parser.add_argument('--teacher-path', type=str, default=r'D:\teacher_model\vgg16-397923af.pth')
    parser.add_argument('--distillation-type', default='none', choices=['none', 'soft', 'hard'], type=str, help="")
    parser.add_argument('--distillation-alpha', default=0.5, type=float, help="")
    parser.add_argument('--distillation-tau', default=1.0, type=float, help="")

    # * Finetuning params
    # D:\PyTorch动手学\GFNet - master_123\GFNet - master_1\checkpoint_epoch20.pth
...
def main(args):
    ...
    if args.arch == 'DFIBNet':
        model = DFIBNet(
            ...
            dims=[32, 64, 128, 256], depths=[2, 2, 6, 2]
        )
    elif args.arch == 'FSFBNet':
        model = FSFBNet(
            img_size=args.input_size, num_classes=num_classes,
            patch_size=16, embed_dim=384, depth=6, mlp_ratio=4,
            norm_layer=partial(nn.LayerNorm, eps=1e-6)
        )
    elif args.arch == 'FSLiteNet':
        model = FSLiteNet(
            img_size=args.input_size, num_classes=num_classes,
    ...
    elif args.arch == 'FSLiteNetF2':
        model = FSLiteNet(
            img_size=args.input_size, num_classes=num_classes,
            patch_size=2, embed_dim=384, fdf_net_depth=12, mlp_ratio=4,
            norm_layer=partial(nn.LayerNorm, eps=1e-6),
            convnext_dims=[32, 64, 128, 256], convnext_depths=[2, 2, 6, 2]
        )
    else:
    ...
    if args.finetune:
        ...
        if args.arch in ['gfnet-ti', 'gfnet-xs', 'gfnet-s', 'gfnet-b']:
            ...
            raise NotImplementedError
        print(f"using checkpoint:{args.finetune} as checkpoint_model")
    ...
    input_size = (3, args.input_size, args.input_size)  # 假设输入为 (3, 224, 224)
    ...
    #
    # # 修改分类层，以适应 CIFAR-10 的 10 个类别
    # teacher_num_classes = 10
    # model.classifier[6] = nn.Linear(4096, teacher_num_classes)
    teacher_model = None
